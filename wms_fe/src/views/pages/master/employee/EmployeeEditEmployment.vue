...
const getFloors = async () => {
    try {
        ...
        if (props.employment.employee_floor && floors.value.length > 0) {
            selectedFloor.value = floors.value.find((floor) => floor.id == props.employment.employee_floor);
            if (!selectedFloor.value) {
                console.warn('No matching floor found for ID:', props.employment.employee_floor);
            }
        }
    } catch (error) {
...
const getSeniorities = async () => {
    try {
        ...
        if (props.employment.employee_seniority && seniorities.value.length > 0) {
            selectedSeniority.value = seniorities.value.find((seniority) => seniority.id == props.employment.employee_seniority);
            if (!selectedSeniority.value) {
                console.warn('No matching seniority found for ID:', props.employment.employee_seniority);
            }
        }
    } catch (error) {
...
const getAgency = async () => {
    try {
        ...
        if (props.employment.employee_agency && agencies.value.length > 0) {
            selectedAgency.value = agencies.value.find((agency_name) => agency_name.id == props.employment.employee_agency);
            if (!selectedAgency.value) {
                console.warn('No matching agency found for ID:', props.employment.employee_agency);
            }
        }
    } catch (error) {
...
const getPosition = async () => {
    try {
        ...
        if (props.employment.employee_position && positions.value.length > 0) {
            selectedPosition.value = positions.value.find((position) => position.id == props.employment.employee_position);
            if (!selectedPosition.value) {
                console.warn('No matching position found for ID:', props.employment.employee_position);
            }
        }
    } catch (error) {
...
const getPositionLvl = async () => {
    try {
        ...
        if (props.employment.employee_position_level && positionlvls.value.length > 0) {
            selectedPositionlvl.value = positionlvls.value.find((position_lvl) => position_lvl.id == props.employment.employee_position_level);
            if (!selectedPositionlvl.value) {
                console.warn('No matching positionlvls found for ID:', props.employment.employee_position_level);
            }
        }
    } catch (error) {
...
const getPositionSts = async () => {
    try {
        ...
        if (props.employment.employee_position_status && positionStatuses.value.length > 0) {
            selectedPositionStatus.value = positionStatuses.value.find((position_sts) => position_sts.id == props.employment.employee_position_status);
            if (!selectedPositionStatus.value) {
                console.warn('No matching position status found for ID:', props.employment.employee_position_status);
            }
        }
    } catch (error) {
